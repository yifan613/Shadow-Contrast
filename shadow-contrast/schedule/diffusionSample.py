...
from model.control import adjust_contrast
import os
class GaussianDiffusion(nn.Module):
    ...
    def forward(self, x_T, cond, img, pre_ori='False'):
        """
        Algorithm 2.
        """
        x_t = x_T
        ...
        for time_step in reversed(range(self.T)):
            ...
            if pre_ori == 'False':
                ...
                # contrast = adjust_contrast(cond_, 1000)
                contrast = adjust_contrast(img, 1000)
                adjusted_contrast = self.model.contrast_adapter(contrast)
                # adjusted_contrast = contrast
                
                if time_step > 0:
                    ori = self.model.denoiser(torch.cat((x_t, cond_), dim=1), t, condition = adjusted_contrast)
                    ...
                    x_t = self.model.denoiser(torch.cat((x_t, cond_), dim=1), t, condition = adjusted_contrast)
