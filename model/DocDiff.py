...
from model.crossattn import CrossAttention
...
class UNet(nn.Module):
    ...
    def __init__(self, input_channels: int = 2, output_channels: int = 1, n_channels: int = 32,
        ...
        * `n_blocks` is the number of `UpDownBlocks` at each resolution
        ...
        # Middle block
        self.cross_attn_mid_before = CrossAttention(out_channels, 3)
        self.cross_attn_mid_after = CrossAttention(out_channels, 3)
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor=torch.tensor([0]).cuda(), condition = None):
        """
        * `x` has shape `[batch_size, in_channels, height, width]`
        * `t` has shape `[batch_size]`
        ...
        # First half of U-Net
        for m in self.down:
            # [B, 32, 1024, 1024] -> [B, 32, 512, 512] -> [B, 64, 512, 512] -> [B, 64, 256, 256] -> [B, 96, 256, 256] -> 128
            x = m(x, t)
            h.append(x)

        # Middle (bottom)
        x = self.cross_attn_mid_before(x, condition)
        x = self.middle(x, t)
        x = self.cross_attn_mid_after(x, condition)
...
#         self.init_predictor = Init_UNet(input_channels//2, output_channels, n_channels, ch_mults, n_blocks, is_noise=False)
...
from model.control import adjust_contrast, Contrast_adapter
...
class DocDiff(nn.Module):
    def __init__(self, input_channels: int = 2, output_channels: int = 1, n_channels: int = 32,
        ...
        self.denoiser = UNet(input_channels, output_channels, n_channels, ch_mults, n_blocks, is_noise=True)
        self.init_predictor = Init_UNet(input_channels//2, output_channels, n_channels, ch_mults, n_blocks, is_noise=False)
        self.contrast_adapter = Contrast_adapter(input_channels//2)
        
        # self.init_predictor = UNet(input_channels, output_channels, 2 * n_channels, ch_mults, n_blocks)

    def forward(self, x, condition, t, diffusion):
        x_ = self.init_predictor(condition, t)  # coarse deshadow.
        
        # to get contrast map adaptively.
        contrast = adjust_contrast(condition, 1000)
        adjusted_contrast = self.contrast_adapter(contrast)
        
        # diffusion process. x is gt, condition is lr.
        noisy_image, noise_ref = diffusion.noisy_image(t, x)
        x__ = self.denoiser(torch.cat((noisy_image,  x_), dim=1), t, condition = adjusted_contrast,)
        return x_, x__, noisy_image, noise_ref
